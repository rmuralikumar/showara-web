...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full bg-[var(--bg-main)]/90 backdrop-blur-md border-b border-[var(--border-subtle)] transition-colors overflow-x-clip">
        <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-1.5 sm:gap-4 min-w-0">
          ...
          <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
            ...
            <Link
              ...
              className="hidden min-[370px]:flex sm:flex items-center gap-1.5 sm:gap-2 p-1.5 sm:px-3 sm:py-1.5 rounded-xl text-xs font-medium bg-[var(--bg-surface-elevated)] border border-[var(--border-subtle)] hover:border-[var(--border-strong)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all touch-target shadow-sm flex-shrink-0"
            >
              <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-[var(--brand-primary)] to-[var(--brand-secondary)] flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                {user.name ? user.name[0] : "U"}
              </div>
              <span className="hidden sm:inline font-medium text-[var(--text-primary)] max-w-[100px] truncate">
                {user.name.split(" ")[0]}
              </span>
            </Link>
          </div>
